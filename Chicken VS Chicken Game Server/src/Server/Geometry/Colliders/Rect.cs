...
namespace GameServer
{
    class Rect : Collider
    {
        ...
        public Rect(Vector2 _position, Vector2 _size, Vector2 _ofset)
        {
            position = _position;
            size = _size;
            ofset = _ofset;
        ...
        {
            return (
                _a.position.x + _a.ofset.x < _b.position.x + _b.ofset.x + _b.size.x &&
                _a.position.x + _a.ofset.x + _a.size.x > _b.position.x + _b.ofset.x &&
                _a.position.y + _a.ofset.y < _b.position.y + _b.ofset.y + _b.size.y &&
                _a.position.y + _a.ofset.y + _a.size.y > _b.position.y + _b.ofset.y
            );
        }
        public Axis CollideWith(Rect _rect)
        {
            if (IsColliding(this, _rect))
            {
                float[] _overlaps = new float[] {
                    _rect.position.x + _rect.ofset.x + _rect.size.x - this.position.x,  // Right overlap
